...
class ProductImportController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try{
            ...
            foreach ( $row_range as $row ) {
                $data[] = [
                    ...
                    'buying_price'  => (float) str_replace(',', '', $sheet->getCell( 'F' . $row )->getValue()), // Remove commas
                    'selling_price' => (float) str_replace(',', '', $sheet->getCell( 'G' . $row )->getValue()), // Remove commas
        ...
        return redirect()
            ->route('products.index')
            ->with('berhasil', 'Data produk berhasil dimasukkan!');
    }
}
